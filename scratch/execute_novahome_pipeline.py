...
def generate_htmls():
    ...
    out_v3 = r"d:\workspaces\PMC_POC\output\NovaHome_stage1_stage2_analysis_v3.html"
    out_v4 = r"d:\workspaces\PMC_POC\output\NovaHome_stage1_stage2_analysis_v4.html"
    out_v5 = r"d:\workspaces\PMC_POC\output\NovaHome_stage1_stage2_analysis_v5.html"
    ...
    v3_content = [
        ...
        "<h1>NovaHome Connect - Detailed Tools (v3)</h1>",
    ...
    for i, tool in enumerate(tools, 1):
        ...
        v3_content.append(f"<div class='card'><p>Detailed generated mock data for {tool} in NovaHome Connect context.</p></div></div>")
    ...
    # --- 3. Modify V4 template for NovaHome ---
    title = soup.find('title')
    if title: title.string = "NovaHome Connect — 종합 진단 보고서 (v4)"
    
    header_title = soup.find('h1')
    if header_title: header_title.string = "❖ NovaHome Connect 프로젝트 심층 진단"

    summaries = {
        "SLIDE 01": "요약: [Stage 1] 명확한 이상징후 식별 및 [Stage 2] Performance Domain 간 다차원적 파급효과 시뮬레이션을 통해, 시스템 관점의 5대 핵심 근본 원인을 도출합니다.",
        "SLIDE 02": "프로젝트 현황: 소셜 로그인(REQ-017) 기능의 임의 드랍과 SDK 지연이라는 징후를 명확하게 포착하고, 거버넌스 붕괴라는 시스템적 결함을 도출했습니다.",
        "SLIDE 04": "파급효과 분석: 부팅 속도(3.5초), CI 테스트 파손 등 주요 KPI 미달이 [Delivery Domain]과 [Quality Domain]에 미치는 파급효과를 심층 분석했습니다.",
        "SLIDE 05": "우선순위: 표면적 증상이 아닌 근본 원인을 파악하고, Proactive 마인드셋에 입각하여 글로벌 런칭 실패 리스크를 방어하기 위한 Top 5 과제를 식별합니다.",
    ...
    if v5_title: v5_title.string = "NovaHome Connect — 통합 보고서 (v5)"
